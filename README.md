### How to use
Download `chatto.html` and use it with an OBS browser source, either by checking the `Local file` option and selecting the file, or by setting its location in the `URL` field prefixed with `file://`. It can also be thrown on a webserver if desires.
